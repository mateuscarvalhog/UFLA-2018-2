...
void Hash::Inserir(const Funcionario& func) {
    Noh* novoNoh = new Noh(func);
    unsigned pos = func.nascimento.Hash(mNroPosicoes);
    mElementos[pos].Adicionar(novoNoh);
}

float Hash::PorcentagemDeUso() const {
    unsigned cont = 0;
    for (int i = 0; i < mNroPosicoes; i++) {
        if (mElementos[i].Tamanho() > 0) {
            cont++;
        }
    }
    return ((float)cont / mNroPosicoes);
}

unsigned Hash::QtdeAniversariantes(const Data& d) {
    int cont = 0;
    unsigned pos = d.Hash(mNroPosicoes);
    Noh* aux = mElementos[pos].mPrimeiro;
    while (aux != NULL) {
